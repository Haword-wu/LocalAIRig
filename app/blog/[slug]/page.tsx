...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    const { frontmatter } = getPost('blog', params.slug)
    return {
      ...
      <header className="mb-8">
        <div className="flex items-center gap-2 mb-4">
          ...
          {frontmatter.tags.map((tag) => (
            <span
              key={tag}
              className="text-xs text-[var(--text-muted)] bg-[var(--surface)] px-2 py-0.5 rounded border border-[var(--border)]"
            >
              #{tag}
            </span>
          ))}

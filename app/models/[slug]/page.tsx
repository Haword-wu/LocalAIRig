...
export async function generateStaticParams() {
  return getSlugs('models').map((slug) => ({ slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    const { frontmatter } = getPost('models', params.slug)
    return {
...
export default function ModelPostPage({ params }: Props) {
  let post
  try {
    post = getPost('models', params.slug)
  } catch {
    notFound()
  }
    ...
    <article className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
      <header className="mb-8">
        <div className="flex items-center gap-2 mb-4">
          ...
          {frontmatter.models?.map((m) => (
            <span
              key={m}
              className="text-xs font-medium px-2 py-0.5 rounded bg-purple-500/10 text-purple-400 border border-purple-500/20"
            >
              {m}
            </span>
          ))}

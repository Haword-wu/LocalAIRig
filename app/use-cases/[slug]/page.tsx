...
export async function generateStaticParams() {
  return getSlugs('use-cases').map((slug) => ({ slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    const { frontmatter } = getPost('use-cases', params.slug)
...
export default function UseCasePostPage({ params }: Props) {
  let post
  try {
    post = getPost('use-cases', params.slug)
  } catch {
    notFound()
  }
